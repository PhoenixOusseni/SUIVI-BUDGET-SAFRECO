...
@section('content')
    <div class="container-fluid px-3 pb-4">
        ...
        <div class="page-hero">
            <div class="d-flex align-items-start justify-content-between flex-wrap gap-2">
                ...
                <div class="d-flex gap-2 flex-wrap">
                    <a href="{{ route('data.prevision.saisi_prevision') }}" class="btn-primary-custom"><i
                            class="fas fa-plus"></i> Saisir</a>
                    <button type="button" class="btn-secondary-custom" data-bs-toggle="modal"
                        data-bs-target="#importModal"><i class="fas fa-file-import"></i> Importer</button>
                    <a href="{{ route('previsions.export', ['year' => request('year', date('Y')), 'ligne_budget_id' => request('ligne_budget_id')]) }}"
                        class="btn-export"><i class="fas fa-file-export"></i> Exporter</a>
                    <a href="{{ route('previsions.print', ['year' => request('year', date('Y')), 'ligne_budget_id' => request('ligne_budget_id')]) }}"
                        class="btn-print" target="_blank"><i class="fas fa-print"></i> Imprimer</a>
                </div>
            </div>
        </div>
        ...
        <div class="toolbar-card mb-4">
            <form method="GET" action="{{ route('gestion_previsions.index') }}"
                ...
                <select name="year" class="form-select" style="width:auto;">
                    @for ($y = $start; $y <= $end; $y++)
                        <option value="{{ $y }}" {{ (int) $currentYear === $y ? 'selected' : '' }}>
                            {{ $y }}</option>
                    @endfor
                </select>
        ...
        <div class="table-card">
            ...
            <div class="table-responsive" style="max-height:70vh; overflow:auto;">
                <table class="table table-bordered table-sm table-exec align-middle mb-0" style="min-width:1600px;">
                    ...
                    <tbody>
                        @forelse($groupedPrevisions as $entry)
                            @php
                                $prevision = $entry->prevision;
                            ...
                            @endphp
                            <tr>
                                <td><span class="status-badge blue">{{ $ligne ? ($ligne->code ?? '—') : ($prevision->ligne_budget_id ? '#'.$prevision->ligne_budget_id : '—') }}</span></td>
                                <td>{{ $ligne ? ($ligne->intitule ?? ($ligne->name ?? '')) : ($prevision->description ?? 'Ligne inconnue') }}</td>
                                ...
                                <td>
                                    @if ($prevision)
                                        <div class="action-group">
                                            <form action="{{ route('gestion_previsions.destroy', $prevision->id) }}"
                                                method="POST" style="display:inline-block;">
                                                @csrf @method('DELETE')
                                                <button type="submit" class="btn-danger-custom"
                                                    onclick="return confirm('Supprimer cette prévision ?')"><i
                        ...
                        @empty
                            <tr>
                                <td colspan="{{ 2 + count($months) + 2 }}" class="text-center py-3 text-muted">Aucune
                                    prévision enregistrée pour {{ $currentYear }}.</td>
                            </tr>
                        @endforelse
