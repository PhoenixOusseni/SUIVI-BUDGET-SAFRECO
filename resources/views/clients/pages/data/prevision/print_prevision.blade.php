...
<head>
    ...
    <title>Prévisions Budgétaires - {{ $year }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
        ...
        <table>
            ...
            <tbody>
                ...
                @forelse($groupedPrevisions as $ligneId => $prevs)
                    @php
                        // Prenons la première prévision (normalement une prévision par ligne / année)
                        ...
                        // Remplir à partir des PrevisionMonth
                    ...
                    <tr class="ligne-row">
                        ...
                        <td class="label-column">
                            {{ $ligne ? $ligne->intitule ?? ($ligne->name ?? '') : $prevision->description ?? '' }}
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="14" style="text-align: center;">Aucune prévision enregistrée pour l'année {{ $year }}.</td>
                    </tr>
                @endforelse

                <!-- Ligne de totaux -->
                @if($groupedPrevisions->isNotEmpty())
                    <tr class="total-row">
        ...
        @if($groupedPrevisions->isNotEmpty())
            <div class="print-info" style="margin-top: 30px; border-top: 1px solid #ddd; padding-top: 15px;">
                ...
                <table style="width: auto; margin-top: 10px;">
                    ...
                    <tr>
                        <td style="border: 1px solid #ddd; padding: 5px; background-color: #e8f5e9; font-weight: bold;">Montants</td>
                        @foreach ($colTotals as $total)
                            <td style="border: 1px solid #ddd; padding: 5px; background-color: #e8f5e9; text-align: right;">
                                {{ number_format($total, 0, ',', ' ') }}
                            </td>
                        @endforeach
                        <td style="border: 1px solid #ddd; padding: 5px; background-color: #e8f5e9; text-align: right; font-weight: bold;">
