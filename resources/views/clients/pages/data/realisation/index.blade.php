...
<div class="container-fluid px-3 pb-4">
    ...
    <div class="page-hero">
        <div class="d-flex align-items-start justify-content-between flex-wrap gap-2">
            <div>
                <p class="hero-title"><i class="fas fa-chart-line me-2"></i>Liste des Réalisations</p>
                <p class="hero-sub">Visualisez les réalisations budgétaires par ligne et par mois</p>
            </div>
            <div class="d-flex gap-2 flex-wrap">
                <a href="{{ route('data.realisation.saisi_realisation') }}" class="btn-primary-custom"><i class="fas fa-plus"></i> Saisir</a>
                ...
                <a href="{{ route('realisations.export', ['year' => request('year', date('Y')), 'ligne_budget_id' => request('ligne_budget_id')]) }}" class="btn-export"><i class="fas fa-file-export"></i> Exporter</a>
                <a href="{{ route('realisations.print', ['year' => request('year', date('Y')), 'ligne_budget_id' => request('ligne_budget_id')]) }}" class="btn-print" target="_blank"><i class="fas fa-print"></i> Imprimer</a>
            </div>
        </div>
    </div>
    ...
    <div class="toolbar-card mb-4">
        <form method="GET" action="{{ route('gestion_realisations.index') }}" class="d-flex gap-2 flex-wrap align-items-center">
            @php
    ...
    <div class="table-card">
        <div class="table-card-header">
            ...
            <div>
                <p class="tch-title">Réalisations — {{ $currentYear }}</p>
                <p class="tch-sub mb-0">{{ count($groupedRealisations) }} ligne(s)</p>
            </div>
        </div>
        @php
        ...
        <div class="table-responsive" style="max-height:70vh; overflow:auto;">
            <table class="table table-bordered table-sm table-exec align-middle mb-0" style="min-width:1600px;">
                ...
                <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <div class="action-group">
                                    <form action="{{ route('gestion_realisations.destroy', $realisation->id) }}" method="POST" style="display:inline-block;">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="btn-danger-custom" onclick="return confirm('Supprimer cette réalisation ?')"><i class="fas fa-trash-alt"></i></button>
                                    </form>
                                </div>
                                @endif
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ 2 + count($months) + 2 }}" class="text-center py-3 text-muted">Aucune réalisation enregistrée pour {{ $currentYear }}.</td>
                        </tr>
                    @endforelse
                    @if(count($groupedRealisations) > 0)
